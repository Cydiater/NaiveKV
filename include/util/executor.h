...
inline void validate_get(const std::map<Key, Value> &kv,
                         kvs::IROEngine::Pointer engine, const TestCase &c) {
  std::string value;
  ...
  if (it != kv.end()) {
    ...
    CHECK_EQ(value, it->second) << "Getting key " << c.key << " expect value "
                                << it->second << ", actual got: " << value;
  } else {
...
inline void validate_put(std::map<Key, Value> &kv, kvs::IEngine::Pointer engine,
                         const TestCase &c) {
...
inline void validate_delete(std::map<Key, Value> &kv,
                            kvs::IEngine::Pointer engine, const TestCase &c) {
  auto it = kv.find(c.key);
  if (it != kv.end()) {
...
inline void validate_visit(const std::map<Key, Value> &kv,
  ...
  auto visitor = [&got, &c, &kv](const Key &key, const Value &value) {
    ...
    CHECK_EQ(it->second, value)
        << "Visiting in range [" << c.lower << ", " << c.upper << ") found key "
        << key << " with unexpected value " << value << ", expect "
        << it->second;
    got++;
  };
...
public:
  ...
  void validate_execute(const std::vector<TestCase> &bench) {
    for (const auto &c : bench) {
      ...
      default: {
        LOG(FATAL) << "invalid op " << (int)c.op << " for a read-only engine";
      }
      }
    }
  }
...
public:
  ...
  EngineValidator(kvs::IEngine::Pointer engine, const std::map<Key, Value> &kv)
      : engine_(engine), kv_(kv) {}
  void execute(const std::vector<TestCase> &bench, size_t from_idx = 0,
    ...
    for (size_t i = from_idx; i < end; ++i) {
      ...
      case Op::kGc: {
        CHECK_EQ(engine_->garbage_collect(), kvs::kSucc) << "Failed to gc.";
        break;
      }
      case Op::kSnapshot: {
      ...
      case Op::kVisit: {
        engine_->visit(c.lower, c.upper, [](const Key &, const Value &) {});
        break;
      }
      case Op::kNoOp: {
        break;
  ...
  const std::map<Key, Value> &get_internal_kv() const { return kv_; }
