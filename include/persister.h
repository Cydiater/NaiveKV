...
public:
  ...
  bool valid() const { return fd_ > 0 && !closed_; }
  ...
  const char *pathname() const { return path_name_.c_str(); }
...
public:
  ...
  File open(const char *pathname, bool o_trunc, bool o_append, bool o_creat) {
    ...
    int ret_fd = ::open(actual_path.c_str(), flag,
                        S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
    if (ret_fd <= 0) {
      std::cerr << "Failed to open file " << pathname << ". errno: " << errno;
    }
    return File(ret_fd, actual_path);
  }

  int unlink(const File &file) { return ::unlink(file.pathname()); }
